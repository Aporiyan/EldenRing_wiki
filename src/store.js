...
// Image manifests for all categories (loaded from local build output)
let weaponImageMap = null;
let armorImageMap = null;
let talismanImageMap = null;
let spellImageMap = null;
...
const IMAGE_MANIFEST_FILES = {
  ...
  talismans: 'talismans-images.json',
  spells: 'spells-images.json',
  'ashes-of-war': 'ashes-of-war-images.json',
  'spirit-ashes': 'spirit-ashes-images.json',
};
...
export async function loadAllData() {
  ...
  loadPromise = (async () => {
    ...
    // Load all image manifests
    const manifestKeys = [
      ['weapon-images.json', v => { weaponImageMap = v; }],
      ['armor-images.json', v => { armorImageMap = v; }],
      ['talismans-images.json', v => { talismanImageMap = v; }],
      ['spells-images.json', v => { spellImageMap = v; }],
      ['ashes-of-war-images.json', v => { ashImageMap = v; }],
      ['spirit-ashes-images.json', v => { spiritAshImageMap = v; }],
    ];
    for (const [file, setter] of manifestKeys) {
      try {
...
export function getWeaponImageUrl(nameEn) {
  if (!nameEn || !weaponImageMap) return null;
  return weaponImageMap[nameEn] || null;
}

const IMAGE_MAP = {
  armaments: () => weaponImageMap,
  armor: () => armorImageMap,
  talismans: () => talismanImageMap,
  spells: () => spellImageMap,
  'ashes-of-war': () => ashImageMap,
  'spirit-ashes': () => spiritAshImageMap,
};

export function getItemImageUrl(key, nameEn) {
  if (!nameEn) return null;
  const mapFn = IMAGE_MAP[key];
  if (!mapFn) return null;
  const map = mapFn();
  if (!map) return null;
  return map[nameEn] || null;
}
